...
[Security](../../index.md)/[OWASP](../index.md)/[2025](index.md)/A10:2021 – Server-Side Request Forgery (SSRF)
...
|----------|--------|---|---|---|---|----|
|Segment remote resource access functionality in separate networks to reduce the impact of SSRF|Follow recommendation in netowrk design|NO|NO|YES|PR????|PR????|
|Enforce “deny by default” firewall policies or network access control rules to block all but essential intranet traffic.|Implement recommendation|NO|NO|YES|PR????|PR????|

### From Application layer

|Mitigation|Solution|DEV|OPS|OCI|NIS|eVZP|
|----------|--------|---|---|---|---|----|
|Sanitize and validate all client-supplied input data|Apply|YES|NO|NO|PR????|PR????|
|Enforce the URL schema, port, and destination with a positive allow list|Apply|YES|NO|YES|PR????|PR????|
|Do not send raw responses to clients|Apply|YES|NO|NO|PR????|PR????|
|Disable HTTP redirections|Apply|YES|NO|NO|PR????|PR????|
